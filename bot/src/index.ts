...
const PROCESS = [
  ...
  "",
  "2. RESOLVE THE PAYER",
  "If the sender is a contract, Finch reads it on-chain: token() and quoteToken() — the token it's built around and the asset it pays out. It also reads its epochCount(), its claim()/distribute() functions, and whether its logic references the Uniswap V4 PoolManager.",
  "",
  "3. CONFIRM THE PATH",
  "If the payer's quoteToken() is the asset you received, the route is confirmed: that asset is the fee currency of token()'s Uniswap V4 pool (behind the Pons Meme Hook) → Pons FeeEscrow → the distributor → an epoch batch that included your wallet. Finch reports the route, not a per-holder rate.",
  "",
  "4. WHAT FINCH DOESN'T CLAIM",
  "Which addresses are in a given epoch's batch is not on-chain-readable — the distributor's logic contract is unverified source, and entry is claim-gated. If quoteToken() doesn't match, Finch says \"not confirmed\" and lists correlational candidates: tokens you hold that also have a Uniswap V4 pool paired against what you received. \"Could be\", not proof.",
  "",
  "WHAT THIS MISSES",
